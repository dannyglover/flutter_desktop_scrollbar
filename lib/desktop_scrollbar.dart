...
// Desktop track > click > jump functionality added by Danny Glover.

library desktop_scrollbar;
...
///
/// You must call [dispose] on this [DesktopScrollbarPainter] when it's no longer used.
///
/// See also:
///
...
class DesktopScrollbarPainter extends ChangeNotifier implements CustomPainter {
  ...
  @override
  bool shouldRepaint(DesktopScrollbarPainter oldDelegate) {
...
/// [CustomScrollView], wrap the scroll view widget in a [DesktopScrollbar] widget.
///
...
///
/// If the child [ScrollView] is infinitely long, the [DesktopScrollbar] will not be
...
///
/// {@tool dartpad}
/// This sample shows a [DesktopScrollbar] that executes a fade animation as
...
/// `controller` for both the [DesktopScrollbar] and the [GridView].
...
class DesktopScrollbar extends StatefulWidget {
  ...
  /// See also:
  ///
  ///   * [DesktopScrollbarState.showScrollbar], an overridable getter which uses
  ...
  /// See also:
  ///
  ///   * [DesktopScrollbarState.enableGestures], an overridable getter which uses
  ///     this value to override the default behavior.
  /// {@endtemplate}
  ...
  @override
  DesktopScrollbarState<DesktopScrollbar> createState() =>
      DesktopScrollbarState<DesktopScrollbar>();
}

/// The state for a [DesktopScrollbar] widget, also shared by the [Scrollbar] and
...
/// scrollbar track.
class DesktopScrollbarState<T extends DesktopScrollbar> extends State<T>
  ...
  @protected
  late final DesktopScrollbarPainter scrollbarPainter;
  ...
  /// theme.
  ///
  /// Defaults to false when [DesktopScrollbar.thumbVisibility] is null.
  @protected
  ...
  /// theme.
  ///
  /// Defaults to true when [DesktopScrollbar.interactive] is null.
  ///
  /// See also:
  ///
  ///   * [DesktopScrollbar.interactive], which overrides the default behavior.
  @protected
  ...
  void initState() {
    ...
    scrollbarPainter = DesktopScrollbarPainter(
  ...
  void _handleTrackTapDown(TapDownDetails details) {
    ...
    final bool scrolledVertically = switch (position.axisDirection) {
      AxisDirection.up || AxisDirection.down => true,
      _ => false
    };
    final double trackSize = scrollbarPainter._trackExtent;
    final double tapPosition = (scrolledVertically)
        ? details.localPosition.dy
        : details.localPosition.dx;
    final double scrollPositionPercentage = (tapPosition / trackSize);
    final double maxScrollExtent = _cachedController!.position.maxScrollExtent;
    final double positionToScroll =
        (scrollPositionPercentage * maxScrollExtent);

    _cachedController!.jumpTo(positionToScroll);
  }
  ...
  /// [DesktopScrollbar].
  ///
  /// Excludes the [DesktopScrollbar] thumb.
  @protected
